...
def format_extract(format_json, input_element, format=None, debug=False, fail='Failure'):
    ...
    for dict_name in ['response_dict', 'embed_dict']:
        if dict_name in format_json:
            for name in format_json[dict_name]:
                try:
                    if isinstance(format_json[dict_name][name], collections.abc.Callable):
                        try:
                            format_dict[name] = format_json[dict_name][name](format_json,
                                                                                input_element)
    ...
    if 'embed' in format_json:
        fm = format_json['embed']
        title = fm.get('title')
        ...
        if isinstance(image, str):
            image = image.format(**format_dict)
        color = fm.get('color')

        embed = discord.Embed(title=title, description=description, url=url, image=image)
